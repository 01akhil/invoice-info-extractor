...
def _ocr_once(job_id: str) -> None:
    ...
    try:
        ...
        if not job:
            ...
            return
        if job.status in (JobStatus.SUCCESS.value, JobStatus.DLQ.value, JobStatus.NEEDS_REVIEW.value):
        ...
        pl_error("ocr", "stage_failed", job_id=job_id, error=str(e), decision="retry_or_NEEDS_REVIEW")
...
def ocr_worker_loop(stop_event, worker_id: int) -> None:
    r = get_redis()
    pl_info("ocr", "worker_ready", worker_id=worker_id, queue=Q_OCR, waits_for="jobs_from_ingest")
