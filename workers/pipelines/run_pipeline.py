...
def _ocr_entry(stop_event, worker_id: int) -> None:
    from workers.core.ocr_worker import ocr_worker_loop
...
def _retry_entry(stop_event: threading.Event) -> None:
    ...
    from workers.retry.retry_ops import retry_scheduler_loop
...
def start_workers(*, run_init_db: bool = True) -> tuple[multiprocessing.Event, threading.Event, list, list, Callable[[], None]]:
    ...
    """
    from workers.config import LLM_THREAD_POOL, OCR_PROCESSES, POST_OCR_THREADS, VALIDATE_THREADS
    from workers.db.session import init_db
    from workers.core.llm_worker import llm_worker_loop
    from workers.core.post_ocr_worker import post_ocr_worker_loop
    from workers.core.validate_worker import validate_worker_loop

    from workers.redis.redis_health import ensure_redis
